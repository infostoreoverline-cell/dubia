...
static const char HTTP_EP[] PROGMEM =
    "https://okopipo-junglelab-vg32.vercel.app/api/ingest";
...
// SLEEP_US e READINGS_PER_SEND sono ora variabili dinamiche caricate da
// LittleFS. Possono essere modificate dall'utente tramite il menu del tasto
// FLASH.
static constexpr uint64_t SLEEP_US_DEFAULT =
    15ULL * 60ULL * 1000000ULL; // 15 minuti (default)
static constexpr uint32_t READINGS_PER_SEND_DEFAULT =
    12; // Invia ogni 3 ore (default)
static uint64_t sleepUs = SLEEP_US_DEFAULT;
static uint32_t readingsPerSend = READINGS_PER_SEND_DEFAULT;

// Valori selezionabili tramite menu (minuti, moltiplicatori invio
// corrispondenti) Opzioni: 5 min (x12 = 1h), 15 min (x12 = 3h), 30 min (x6 =
// 3h), 60 min (x3 = 3h)
...
static_assert(sizeof(RtcWifiCache) == 12,
              "RtcWifiCache deve essere di 12 byte");
static RtcWifiCache rtcWifi;

static constexpr uint8_t RTC_WIFI_OFFSET = 4;
static constexpr uint8_t RTC_RF_OFFSET = 3;

static_assert(RTC_FLAG_OFFSET * 4u >= sizeof(RtcCounter),
              "RTC Flag sovrapposto a RtcCounter!");
static_assert(RTC_RF_OFFSET * 4u >= (RTC_FLAG_OFFSET * 4u + 4u),
              "RTC RF sovrapposto a Flag!");
static_assert(RTC_WIFI_OFFSET * 4u >= (RTC_RF_OFFSET * 4u + 4u),
              "RTC WiFi sovrapposto a RF!");
...
static FLASH_FN bool readRTC() {
  ESP.rtcUserMemoryRead(0, reinterpret_cast<uint32_t *>(&rtcCounter),
                        sizeof(rtcCounter));
...
static FLASH_FN void writeRTC() {
  ...
  ESP.rtcUserMemoryWrite(0, reinterpret_cast<uint32_t *>(&rtcCounter),
                         sizeof(rtcCounter));
}
...
static void writeRTCFlag(uint32_t v) {
  rtcFlagWord = v;
...
static void writeRTCRfPrepared(uint32_t v) {
  rtcRfPrepared = v;
  ESP.rtcUserMemoryWrite(RTC_RF_OFFSET, &rtcRfPrepared, sizeof(rtcRfPrepared));
}

static FLASH_FN bool readRTCWifi() {
  ESP.rtcUserMemoryRead(RTC_WIFI_OFFSET, reinterpret_cast<uint32_t *>(&rtcWifi),
                        sizeof(rtcWifi));
...
static FLASH_FN void writeRTCWifi() {
  ...
  ESP.rtcUserMemoryWrite(
      RTC_WIFI_OFFSET, reinterpret_cast<uint32_t *>(&rtcWifi), sizeof(rtcWifi));
}
...
static FLASH_FN void loadConfig() {
  ...
  sleepUs = SLEEP_US_DEFAULT;
  readingsPerSend = READINGS_PER_SEND_DEFAULT;
  if (LittleFS.begin()) {
    if (LittleFS.exists(FPSTR(CONFIG_FILE))) {
      ...
      if (f) {
        ...
        if (sendRds.length() > 0) {
          ...
          if (readingsPerSend == 0 || readingsPerSend > 100)
            readingsPerSend = READINGS_PER_SEND_DEFAULT;
        } else {
          // Se riga 5 assente, imposta il valore di default accoppiato
          // all'intervallo caricato
...
static FLASH_FN void saveConfig(const String &ssid, const String &pass,
                                const String &ep) {
  if (LittleFS.begin()) {
...
// =============================================================================
// Nota: questa funzione e' chiamata all'inizio di setup() PRIMA del deep sleep
// check. Usa una funzione OLED inline per evitare dipendenze circolari con
// runWifiPortal.
static FLASH_FN void runFlashButtonMenu() {
  ...
  bool dispOk = display.begin(SSD1306_SWITCHCAPVCC, DISP_PRI) ||
  ...
  auto showSleepMenuOled = [&]() {
    ...
    display.setTextColor(SSD1306_WHITE);
    display.setCursor(0, 20);
    display.print(F("Click: cambia valore"));
    display.setCursor(0, 34);
    display.print(F("Attesa 3s: avanti"));
    char buf[16];
  ...
  for (uint8_t i = 0; i < NUM_SEND_OPTS; i++) {
    if (SEND_OPTS[i] == readingsPerSend) {
      sendIdx = i;
      break;
    }
  }
  ...
  auto showSendMenuOled = [&]() {
    ...
    display.print(F("Click: cambia valore"));
    display.setCursor(0, 34);
    display.print(F("Attesa 3s: salva"));
    char buf[24];
  ...
  uint64_t tempSleepUs = (uint64_t)SLEEP_OPTIONS_MIN[sleepIdx] * 60ULL * 1000000ULL;
  uint32_t tempReadingsPerSend = SEND_OPTS[sendIdx];
  loadConfig(); // ricarica ssid/pass/ep per poter salvare
  sleepUs = tempSleepUs;
  readingsPerSend = tempReadingsPerSend;
  ...
  if (dispOk) {
    ...
    display.display();
    {
      const uint32_t _t = millis();
      while (millis() - _t < 2500UL)
        yield();
    }
    turnOledOff();
  }
...
static FLASH_FN void runWifiPortal() {
  ...
  if (n_networks <= 0) {
    wifiListHtml =
        F("<p style='color:#94a3b8;font-size:14px;text-align:center;'>Nessuna "
          "rete trovata.</p>");
  } else {
    ...
    for (int i = 0; i < limit; ++i) {
      ...
      String sc = (rssi >= -67) ? "sig-strong"
                                : ((rssi >= -75) ? "sig-medium" : "sig-weak");
      String lk = (enc != ENC_TYPE_NONE) ? " \xF0\x9F\x94\x92" : "";
      String es = ssid;
      es.replace("\\", "\\\\");
      es.replace("'", "\\'");
      es.replace("\"", "\\\"");
      wifiListHtml += "<div class='wifi-item' onclick='selectWifi(\"" + es +
                      "\")'><span class='wifi-ssid'>" + ssid + lk +
                      "</span><span class='wifi-rssi " + sc + "'>" +
                      String(rssi) + " dBm</span></div>";
    }
    wifiListHtml += F("</div>");
  }
  ...
  webServer.on("/", [&webServer, &wifiListHtml]() {
    String html = F(
        ...
        "initial-scale=1.0'><title>Config</"
        "title><style>body{background:linear-gradient(135deg,#0f172a,#1e1b4b);"
        "color:#f8fafc;font-family:sans-serif;display:flex;justify-content:"
        "center;align-items:center;min-height:100vh;margin:0;padding:20px;box-"
        "sizing:border-box}.card{background:rgba(255,255,255,0.05);border:1px "
        "solid "
        "rgba(255,255,255,0.1);border-radius:16px;padding:30px;width:100%;max-"
        "width:400px}h2{margin-top:0;text-align:center;color:#60a5fa}.group{"
        "margin-bottom:16px}label{display:block;font-size:13px;color:#94a3b8;"
        "margin-bottom:5px}input{width:100%;padding:10px;background:rgba(15,23,"
        "42,0.6);border:1px solid "
        "rgba(255,255,255,0.15);border-radius:8px;color:white;font-size:15px;"
        "box-sizing:border-box}button{width:100%;padding:13px;background:#"
        "3b82f6;color:white;border:none;border-radius:8px;font-size:15px;font-"
        "weight:600;cursor:pointer}.wifi-list{max-height:130px;overflow-y:auto;"
        "border:1px solid "
        "rgba(255,255,255,0.1);border-radius:8px;background:rgba(15,23,42,0.4);"
        "margin-bottom:12px;padding:4px}.wifi-item{display:flex;justify-"
        "content:space-between;padding:7px "
        "9px;cursor:pointer;border-radius:6px;font-size:13px}.wifi-item:hover{"
        "background:rgba(59,130,246,0.15)}.sig-strong{color:#10b981}.sig-"
        "medium{color:#f59e0b}.sig-weak{color:#ef4444}</style><script>function "
        "selectWifi(s){document.getElementById('ssid').value=s;document."
        "getElementById('pass').focus();}</script></head><body><div "
        "class='card'><h2>Configurazione Wi-Fi</h2><form action='/save' "
        "method='POST'><div class='group'><label>Reti "
        "rilevate</label>{WIFI_LIST}</div><div class='group'><label "
        "for='ssid'>SSID</label><input type='text' id='ssid' name='ssid' "
        "required value='{SSID}'></div><div class='group'><label "
        "for='pass'>Password</label><input type='password' id='pass' "
        "name='pass' value='{PASS}'></div><div class='group'><label "
        "for='ep'>Endpoint API</label><input type='text' id='ep' name='ep' "
        "required value='{EP}'></div><button "
        "type='submit'>Salva</button></form></div></body></html>");
    ...
    if (ssid.length() > 0 && ep.length() > 0) {
      ...
      webServer.send(
          ...
          F("<!DOCTYPE html><html><body "
            "style='background:#0f172a;color:#f8fafc;display:flex;justify-"
            "content:center;align-items:center;height:100vh;font-family:sans-"
            "serif;'><div style='text-align:center'><h2 "
            "style='color:#10b981'>Salvato!</h2><p>Il dispositivo si "
            "riavviera' tra poco.</p></div></body></html>"));
      {
        const uint32_t _t = millis();
        while (millis() - _t < 2000UL)
          yield();
      }
      // Spegne lo schermo OLED prima del riavvio per evitare che rimanga
      // "congelato" acceso durante lo sleep successivo
      turnOledOff();
      ESP.restart();
    } else {
...
static FLASH_FN bool detectSensorPins() {
  ...
  const uint8_t pinPairs[][2] = {{4, 5}, {5, 4}, {14, 12}, {12, 14}};
...
static FLASH_FN bool readSensor(int16_t &t10, int16_t &h10) {
  ...
  if (!initOk) {
    DBG_PRINTLN(F("[SHT40] ERR: init dopo 5 tentativi"));
    return false;
  }
  ...
  DBG_PRINTF("[SHT40] T=%s%d.%d C  H=%d.%d%%\n", (t10 < 0 ? "-" : ""),
             abs(t10) / 10, abs(t10) % 10, h10 / 10, h10 % 10);
  return true;
}
...
static FLASH_FN void saveMeasurement(int16_t t10, int16_t h10) {
  if (!LittleFS.begin()) {
    DBG_PRINTLN(F("[FS] ERR: mount"));
    return;
  }
  File f = LittleFS.open(FPSTR(DATA_FILE), "a");
  if (!f) {
    DBG_PRINTLN(F("[FS] ERR: open"));
    LittleFS.end();
    return;
  }
...
static FLASH_FN void sendWiFiData() {
  ...
  if (cacheValid) {
    ...
    WiFi.begin(wifiSsid.c_str(), wifiPass.c_str(), rtcWifi.channel,
               rtcWifi.bssid);
  } else {
  ...
  while (WiFi.status() != WL_CONNECTED) {
    ...
    if (el >= WIFI_TIMEOUT_MS) {
      DBG_PRINTLN(F("[WiFi] Timeout"));
      wifiOff();
      return;
    }
    yield();
  ...
  if (!LittleFS.begin()) {
    DBG_PRINTLN(F("[FS] ERR mount per invio"));
    wifiOff();
    return;
  }
  ...
  client.setBufferSizes(
      4096, 512); // 4096 RX sufficiente per API Vercel (riduce heap usage)
  HTTPClient http;
  ...
  if (http.begin(client, httpEndpoint)) {
    ...
    char intervalStr[16];
    snprintf(intervalStr, sizeof(intervalStr), "%u",
    ...
    if (code < 0) {
      ...
      char eb[100];
      int e = client.getLastSSLError(eb, sizeof(eb));
      if (e)
        DBG_PRINTF("[SSL] %d - %s\n", e, eb);
    }
...
static FLASH_FN void runDisplayCycle(uint32_t durationMs = 10000UL) {
  ...
  if (!display.begin(SSD1306_SWITCHCAPVCC, DISP_PRI) &&
      !display.begin(SSD1306_SWITCHCAPVCC, DISP_FALL)) {
    DBG_PRINTLN(F("[OLED] ERR: init fallita"));
    return;
  }
  ...
  while (millis() - t0 < durationMs) {
    ...
    if (readSensor(t10, h10)) {
      ...
      display.print(F("Tieni FLASH per Menu"));
      display.display();
    }
...
void setup() {
  ...
  if (!readRTC()) {
    DBG_PRINTLN(F("[RTC] CRC invalido - azzerato"));
    rtcCounter.counter = 0;
    writeRTC();
  }
  if (rtcCounter.counter > 10000u)
    rtcCounter.counter = 0;
  ...
  if (isFirstManualReset) {
    ...
    while (millis() - t_win < DISPLAY_WIN_MS) {
      // Se l'utente preme FLASH durante la finestra di standby, entriamo nel
      // menu
      if (digitalRead(PIN_FLASH) == LOW) {
        runFlashButtonMenu();
      }
  ...
  if (autoWake && rtcRfPrepared == 1) {
    ...
    delay(1);
    sendWiFiData();
    uint32_t elMs = millis();
    uint64_t slMs = sleepUs / 1000ULL;
    uint64_t actUs =
        (slMs > (uint64_t)elMs) ? (slMs - (uint64_t)elMs) * 1000ULL : 0ULL;
  ...
  if (!sensorOk) {
    DBG_PRINTLN(
        F("[SHT40] Primo ciclo fallito. Pausa 500ms e secondo tentativo..."));
    delay(500);
